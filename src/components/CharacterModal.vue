  ...
  <div class="character-modal" v-if="character">
    ...
    <div class="modal-character-header">
      <div class="avatar-container">
        <div class="avatar" :style="{ backgroundImage: 'url('+ character.image +')' }"></div>
      </div>
      ...
      <div class="character-content">
            ...
            <small>
              <b-tooltip position="is-right" multilined>
                ...
                <template v-slot:content>
                    <b>The gender of the character ('Female', 'Male', 'Genderless' or 'unknown')</b>.
                </template>
              </b-tooltip>
              Gender:
            </small>
       ...
       <div class="column is-one-third">
        ...
        <div class="container character-cards character-cards-just-two is-mobile">
          <div 
          class="character-card" 
          v-for="(randomCharacter, index) in randomCharacters"
          :key="index"
          @click="selectCharacter(randomCharacter.url)">
          <div class="image" :style="{ backgroundImage: 'url('+ randomCharacter.image +')' }">
            ...
            <div class="character-info">
              <div class="status">
                <b-icon
                :type="{'is-success': randomCharacter.status === 'Alive', 'is-danger': randomCharacter.status !== 'Alive'}"
                pack="mdi"
                icon="circle"
                size="is-small">
                </b-icon>
  ...
  methods: {
    async getCharacter(url) {
      ...
      axios.get(url, this.config)
        ...
        .catch(() => {
          this.errorToast("Ha ocurrido un error al buscar la información del personaje seleccionado☹️");
   ...
   async selectCharacter(url) {
    ...
    async getRandomCharacters() {
      ...
      axios.get(`https://rickandmortyapi.com/api/character/${random[0]},${random[1]},${random[2]}`, this.config)
        .then(res => {
        ...
        .catch(() => {
          this.errorToast("Ha ocurrido un error al buscar personajes de interés, por favor intenta de nuevo.");
      });
    },
      ...
      switch (type) {
        ...
        case "random":
          this.randomCharacters.forEach((character, index) => {
            this.randomCharacters[index].episode = this.episodes.find(episode => {
